...
pub(crate) mod tests {
    ...
    fn test_balloon_virtio_device_active() {
        ...
        // Success
        {
            ...
            let queues = vec![
                VirtioQueueConfig::<QueueStateSync>::create(128, 0).unwrap(),
                VirtioQueueConfig::<QueueStateSync>::create(128, 0).unwrap(),
                VirtioQueueConfig::<QueueStateSync>::create(128, 0).unwrap(),
            ];
    ...
    fn test_balloon_epoll_handler_process_report_queue() {
        ...
        assert_eq!(handler.process_reporting_queue(), false);
        ...
        //Success
        assert_eq!(handler.process_reporting_queue(), true);
    }
    ...
    fn test_balloon_epoll_handler_process_queue() {
        ...
        // invalid idx
        {
            ...
            assert_eq!(handler.process_queue(10), false);
        }
        // INFLATE_QUEUE_AVAIL_EVENT
        {
            let vq = VirtQueue::new(GuestAddress(0), m, 16);
            ...
            assert!(queue_config.generate_event().is_ok());
            handler.inflate = queue_config;
            assert_eq!(handler.process_queue(INFLATE_QUEUE_AVAIL_EVENT), true);
        }
        // DEFLATE_QUEUE_AVAIL_EVENT
        {
            let vq = VirtQueue::new(GuestAddress(0), m, 16);
            ...
            assert_eq!(handler.process_queue(DEFLATE_QUEUE_AVAIL_EVENT), true);
        }
    }
}
