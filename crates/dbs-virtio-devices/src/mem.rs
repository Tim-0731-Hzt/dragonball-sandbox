...
impl MemTool {
    fn virtio_mem_valid_range(config: &VirtioMemConfig, addr: u64, size: u64) -> bool {
        ...
        if addr % config.block_size as u64 != 0 || size % config.block_size as u64 != 0 {
            return false;
        }
            ...
            if let Some((map_regions, factory)) = multi_regions {
                ...
                if let Some(first_index) = first_unmap_index {
                    ...
                    let region_len = ((regions_num - 1) << VIRTIO_MEM_MAP_REGION_SHIFT)
                        + if last_index + 1 == map_regions.len() as usize {
                            config.region_size
...
fn get_map_regions_num(region_size: u64) -> usize {
    ((region_size >> VIRTIO_MEM_MAP_REGION_SHIFT)
        + if region_size & VIRTIO_MEM_MAP_REGION_MASK > 0 {
            1
        } else {
            0
        }) as usize
...
impl<AS: GuestAddressSpace> Mem<AS> {
    ...
    pub fn new(
        ...
        config.block_size = VIRTIO_MEM_DEFAULT_BLOCK_SIZE as u64;
...
pub(crate) mod tests {
    ...
    fn test_mem_request_parse() {
        ...
        // write only request type descriptor
        {
            ...
            assert!(
                match Request::parse(&mut q.iter(m).unwrap().next().unwrap(), m) {
                    Err(MemError::UnexpectedWriteOnlyDescriptor) => true,
                    _ => false,
                }
            );
        }
        // desc len error
        {
            ...
            assert!(
                match Request::parse(&mut q.iter(m).unwrap().next().unwrap(), m) {
                    Err(MemError::InvalidRequest) => true,
                    _ => false,
                }
            );
        }
        // desc chain too short
        {
            ...
            assert!(
                match Request::parse(&mut q.iter(m).unwrap().next().unwrap(), m) {
                    Err(MemError::DescriptorChainTooShort) => true,
                    _ => false,
                }
            );
        }
        // unexpected read only descriptor
        {
            ...
            assert!(
                match Request::parse(&mut q.iter(m).unwrap().next().unwrap(), m) {
                    Err(MemError::UnexpectedReadOnlyDescriptor) => true,
                    _ => false,
                }
            );
        }
        // desc len too short
        {
            ...
            assert!(
                match Request::parse(&mut q.iter(m).unwrap().next().unwrap(), m) {
                    Err(MemError::DescriptorLengthTooSmall) => true,
                    _ => false,
                }
            );
        }
    ...
    fn test_mem_tool_valid_range() {
        let mut config = VirtioMemConfig::default();
        config.block_size = 0x100;
        config.addr = 0x1000;
        config.usable_region_size = 0x1000;

        // address not properly aligned.
        assert_eq!(MemTool::virtio_mem_valid_range(&config, 0x14, 0x100), false);
        assert_eq!(MemTool::virtio_mem_valid_range(&config, 0x100, 5), false);

        // unreasonable size.
        assert_eq!(
            MemTool::virtio_mem_valid_range(&config, 0x1000, i32::MAX as u64),
            false
        );
        assert_eq!(MemTool::virtio_mem_valid_range(&config, 0x1000, 0), false);

        // start address not in usable range.
        assert_eq!(
            MemTool::virtio_mem_valid_range(&config, 0x200, 0x200),
            false
        );
        assert_eq!(
            MemTool::virtio_mem_valid_range(&config, 0x3000, 0x200),
            false
        );

        // end address not in usable range.
        assert_eq!(
            MemTool::virtio_mem_valid_range(&config, 0x1000, 0x2000),
            false
        );

        // success
        assert_eq!(
            MemTool::virtio_mem_valid_range(&config, 0x1000, 0x500),
            true
        );
    }
    ...
    fn test_mem_tool_check_bitmap() {
        ...
        // true
        assert_eq!(
            MemTool::virtio_mem_check_bitmap(bit_index, nb_blocks, &mem_state, plug),
            true
        );
    ...
    fn test_mem_tool_set_bitmap() {
        ...
        assert_eq!(mem_state[2], true);
        assert_eq!(mem_state[3], true);
    }
    ...
    fn test_mem_virtio_device_normal() {
        ...
        VirtioDevice::<Arc<GuestMemoryMmap<()>>, QueueStateSync, GuestRegionMmap>::set_acked_features(&mut dev, 2, 0);
        assert_eq!(
            VirtioDevice::<Arc<GuestMemoryMmap<()>>, QueueStateSync, GuestRegionMmap>::get_avail_features(&dev, 2),
            0,
        );
    ...
    fn test_mem_virtio_device_activate() {
        ...
        // queue length error
        {
            ...
            match result {
                Err(ActivateError::InvalidParam) => assert!(true),
                _ => assert!(false),
            }
        }
        // fail because map_regions should not be empty
        {
            ...
            let mem = GuestMemoryMmap::from_ranges(&[(GuestAddress(0), 0x10000)]).unwrap();
            let queues = vec![VirtioQueueConfig::<QueueStateSync>::create(128, 0).unwrap()];
            ...
            let result = dev.activate(config);
            match result {
                Err(ActivateError::InternalError) => assert!(true),
                _ => assert!(false),
            }
        }
    ...
    fn test_mem_virtio_device_remove() {
        ...
        VirtioDevice::<Arc<GuestMemoryMmap<()>>, QueueStateSync, GuestRegionMmap>::remove(&mut dev);
    ...
    fn test_mem_epoll_handler_process_queue() {
        ...
        //fail to parse available descriptor chain
        {
            ...
            assert_eq!(handler.process_queue(0), true);
        }
        //success
        {
            let vq = VirtQueue::new(GuestAddress(0), m, 16);
